...
This first regression is shown in Table 1 below. Standard error are clustered at the multi-way group level. We supply the optional argument `cmethod = 'cgm2'` which implements multi-way clustering as in Stata and Julia. In particular, clustering is implemented as follows
...
Next we observe the interaction term with bank capital...
...
We remove some controls in order to determine the robustnes...
...
interaction_term_names = sprintf("mp_x_%s", interaction_terms_bank_level)
dt[,(interaction_term_names):=lapply(.SD, function(i) i*mpshock_l),.SDcols=interaction_terms_bank_level]
indep_vars <- colnames(dt)[!colnames(dt) %in% c(dep_vars,"bankid","date_q", "bdepo_l","bcet1_l","blar_l","bsize_l")]
output <- lapply(
  1:length(dep_vars),
  function(i) {
    col_names <- c(dep_vars[i], indep_vars)
